...
if response.status_code == 200:
    ...
    for row in table.find_all('tr')[1:]:  #pula a primeira linha
        ...
        if cols:  # Só adiciona se a linha não estiver vazia
            data.append(cols)
    
    #debug: mostra o número de colunas nos dados
    if data:
        print(f"colunas nos dados: {len(data[0])}")
        print("primeira linha de dados:", data[0])
    
    # Se não houver cabeçalhos ou o número estiver errado, ajusta
    if not headers and data:
        headers = [f"Coluna_{i+1}" for i in range(len(data[0]))]
    
    # Verifica se o número de cabeçalhos corresponde aos dados
